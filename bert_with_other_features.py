...
class CustomClassifier(nn.Module):
    ...
    def forward(self, x):
        if x.size(1) > self.basic_input_size: # pass only the output of BERT from dropout and not the audio processing features if there are any
            selected_cols = x[:, :self.basic_input_size]
            x[:, :basic_input_size] = self.dropout(selected_cols)
        else:
            x = self.dropout(x)
...
for model_name, basic_input_size, NUM_EPOCHS, BATCH_SIZE in zip(model_names_list, basic_input_sizes, NUM_EPOCHS_LIST, BATCH_SIZES):
    ...
    for current_activation_function in activation_functions:
        for dataset_description in dataset_description_list:
            CURRENT_CLASSIFIER_CHECKPOINT_FILE = f'{OUTPUT_FOLDER}/custom_bert_custom_classifier_{current_activation_function}_{model_name}_{dataset_description}.pth'
